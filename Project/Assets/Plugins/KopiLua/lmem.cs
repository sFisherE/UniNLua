...
namespace KopiLua
{
	public partial class Lua
	{
		...
		public static T[] LuaMReallocVector<T>(LuaState L, ref T[] v, int oldn, int n)
		{
            ...
            Unity3DIntegration.Assert((v == null && oldn == 0) || (v.Length == oldn));
		...
		public static object LuaMRealloc<T>(LuaState L, T[] old_block, int new_size)
		{
			...
			if (CanIndex(typeof(T)))
				for (int i = 0; i < new_size; i++)
				{
                    ...
                    Unity3DIntegration.Assert(elem != null, String.Format("Need to derive type {0} from ArrayElement", typeof(T).ToString()));
					elem.SetIndex(i);
